...
them into a shared pool, and the app picks the next movie from the pool at
...
2. Pool: each member promotes up to 3 movies from their stash into the shared
   pool.
3. Pick: on movie night, the app picks one movie from the pool at random.
4. Watched: mark the current pick as watched and it moves into the watched
   library, together with its picker and the watch date.
5. Stats: watch counts per member, activity by weekday and hour, top genres,
   most-watched directors and actors.

## The tabs

- Movies: the current pick with its poster and details, the button that picks
  the next movie, the shared pool, and the watched library (searchable by title
  or picker). Every movie opens a detail view with its overview, credits and a
  cast strip.
...
- Stats: the watch history over a selectable time window. A member leaderboard,
  ...
  page can be filtered by genre, release year or decade, picker, and specific
  actors or crew.
