...
- **Users** — members, each with their pool slots (max 3) and a searchable stash;
  add via TMDB search.
- **Stats** — watch stats over a time window (member leaderboard — every member
  ...
  them in the matching people filter and drills the whole page down (every
  aggregate is computed server-side over the filtered subset); each card also
  carries a corner link to the person's TMDB page.
