...
func (r *SqliteRadarrRepository) ListInstances(ctx context.Context, includeArchived bool) ([]RadarrInstance, error) {
	...
	rows, err := r.pool.Read.QueryContext(ctx, `
		SELECT id, name, base_url, encrypted_api_key, revision, state, state_reason,
		       last_checked_at, created_at, updated_at, archived_at
		FROM radarr_instances
		`+where+`
...
func readRadarrInstance(ctx context.Context, q queryRower, id int64) (RadarrInstance, error) {
	instance, err := scanRadarrInstance(q.QueryRowContext(ctx, `
		SELECT id, name, base_url, encrypted_api_key, revision, state, state_reason,
		       last_checked_at, created_at, updated_at, archived_at
		FROM radarr_instances
		WHERE id = ?
...
func scanRadarrPreset(row rowScanner) (RadarrPreset, error) {
	...
	if err := row.Scan(
		...
		&validatedAt, &createdAt, &updatedAt, &archivedAt,
	); err != nil {
